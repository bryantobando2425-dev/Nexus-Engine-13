...
router.post("/dream", async (req, res) => {
  try {
    const { character, emotionalClimate, innerVoiceLog = [], recentEvents = [], era, aiProvider } = req.body;
...
Empieza directamente con el sueño, sin introducción. Usa lenguaje poético y evocador.`;
    ...
    const { text: dreamText } = await generateWithProvider(provider, "state", prompt);
...
router.post("/state-update", async (req, res) => {
  try {
    const { character, worldState, recentHistory = [], existingNPCs = [], facciones = [], realisticAttributes, descriptors, inventory = [], currency, era, memoriaNarrador, sections = [], manualInstructions = '', customSections = [], aiProvider } = req.body;
    const provider = resolveAIProvider(aiProvider);
...

{
  ...
  "summary": "Resumen de los cambios realizados en una oración."
    ...
    const { text: stateRaw } = await generateWithProvider(provider, "state", prompt);
...
router.post("/suggest", async (req, res) => {
  try {
    const { fieldPath, currentValue, suggestion, context, aiProvider } = req.body;
    const provider = resolveAIProvider(aiProvider);
...
Solo JSON puro, sin markdown.`;
    ...
    const { text: suggestRaw } = await generateWithProvider(provider, "state", prompt);
    let result: any = {};
    try {
...
{ "resumen": "<nuevo resumen completo aquí>" }`;
    ...
    const { text: memoryRaw } = await generateWithProvider(provider, "state", prompt);
    let resumen = '';
    try {
