...
import { generateWithProvider, resolveAIProvider } from "../aiProviders";

const router: IRouter = Router();

router.post("/", async (req, res) => {
  try {
    const { type, context, aiProvider } = req.body;
    const provider = resolveAIProvider(aiProvider || context?.aiProvider);
...
Actions must be in Spanish. Keep them brief (5-10 words). They should be contextual to the situation, not generic.`;
    ...
    const { text: raw } = await generateWithProvider(provider, "state", prompt, systemPrompt);
